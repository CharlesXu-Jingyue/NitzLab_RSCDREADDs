...
%         end
        if watchKeysFlag
            ...
            if objective == 0 & nClicks == 4 & strcmpi(keyPressed,'return')
                ...
                objective = 1;
                nClicks = 0;
                instructions = 'PATHS 1-4 - Mark the start line with two clicks.';
                moveTimes(1);
                watchKeysFlag = 0;
            end
            drawPoints;
...
%%  Program Control for autoPosTracker
    ...
    function zoneMarker
        ...
        if (length(timeStamps) == 4 && nClicks == 18) || nClicks == 36
            runFinder;
    ...
    function runFinder
        ...
        if length(timeStamps) == 6
            sessFilter = zeros(length(pixelDvt),18);
            sessFilter(timeStamps(3):timeStamps(4),1:9) = 1;
            sessFilter(timeStamps(5):timeStamps(6),10:18) = 1;
        ...
        for iSample = timeStamps(3):timeStamps(4)
            ...
            elseif dvtZoneMarked(iSample,6)
                [runType, line] = find(dvtZoneMarked(iSample+1:timeStamps(4),:)',1);
                if ~isempty(runType)
                    if runType == 7
            ...
            elseif dvtZoneMarked(iSample,8)
                [runType, line] = find(dvtZoneMarked(iSample+1:timeStamps(4),:)',1);
                if ~isempty(runType)
                    if runType == 9
